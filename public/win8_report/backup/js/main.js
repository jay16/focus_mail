...
$(window).hashchange(function(){
	$hashed.parts = window.location.hash.replace("#!/","").replace("!/","").replace("#!","").replace("#","").split("&"); 
...
$(window).resize(function(){
	...
	if(windowWidth < 3.2){
		...
		setTileOpacity();	
	}else if(autoRearrangeTiles && windowWidth < rearrangeTreshhold+1.5){		
		...
		if(!$("body").hasClass("full")){
			...
			if($page.current == "home"){
				...
				$("#tileContainer").children(".groupTitle").each(function(){
	...
	/*Check mousewheel */
	if($page.current != 'home' || $page.layout == "column" || (disableGroupScrollingWhenVerticalScroll && $(document).height()>$(window).height())){	/*Scrolling on pages and home */
